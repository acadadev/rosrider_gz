...
def generate_launch_description():
    ...
    gz_sim = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            ...
            launch_arguments={
                'gz_args': f'-r {os.path.join(pkg_project_gazebo, "worlds", "world_maze.sdf")}'
        }.items(),
    )
    ...
    # notice: since we are using the world bridge
    bridge = Node(
        package='ros_gz_bridge',
        executable='parameter_bridge',
        parameters=[{
            'config_file': os.path.join(pkg_project_bringup, 'bridge', 'next_gz_singular_world_bridge.yaml'),
